...
@Component({
  ...
  templateUrl: './caption-demo.html',
  styleUrls: ['./caption-demo.css']
})
export class CaptionDemo {
  public documentation = {
    ...DOCUMENTATION.directives.table.captiom,
    usage: DOCUMENTATION.usage.replace("$s", "<ng-template epCaption>"),
    props: DOCUMENTATION.props.replace("$s", "<ng-template epCaption>"),
  }
  public data = [
    {
      prop: "ng-template",
    ...
    },
    {
      prop: "epCaption",
      type: "Directive",
      description: "Directiva que marca el ng-template como bloque de título de la tabla para que el componente padre pueda detectarlo y renderizarlo.",
  ...
  get codeTemplate() {
    const raw = {
      ...
      children: [{
        tag: "ng-template",
        attrs: { epCaption: "$_" },
        children: ["<!-- Proyección de contenido con epCaption -->"],
      }]
    };
  ...
  get codeScript() {
    return EpDedent(EpBuildDocScriptBase({
      imports: [{ from: '@ep/directives', imports: ["EpTableCaptionDirective",] }],
