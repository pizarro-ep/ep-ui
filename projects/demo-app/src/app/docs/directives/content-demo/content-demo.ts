...
@Component({
  ...
  styleUrls: ['./content-demo.css']
})
export class ContentDemo {
  public documentation = {
    ...DOCUMENTATION.directives.content,
    usage: DOCUMENTATION.usage.replace("$s", "<ng-template epContent>"),
    props: DOCUMENTATION.props.replace("$s", "<ng-template epContent>"),
  }
  public data = [
    {
      prop: "ng-template",
    ...
    },
    {
      prop: "epContent",
      type: "Directive",
      description: "Directiva que marca el ng-template como bloque de contenido para que el componente padre pueda detectarlo y renderizarlo.",
  ...
  get codeTemplate() {
    const raw = {
      tag: "ng-template",
      attrs: { epContent: "$_" },
      children: ["<!-- Proyección de contenido con epContent -->"],
    };
    return EpDedent(EpBuildNode(raw));
  }
  get codeScript() {
    return EpDedent(EpBuildDocScriptBase({
      imports: [{ from: '@ep/directives', imports: ["EpContentDirective",] }],
